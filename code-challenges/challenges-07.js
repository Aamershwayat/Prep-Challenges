...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
            if (arr[i].tech == "JS") {
                abuKhadeeja.javaScript_Devs = abuKhadeeja.javaScript_Devs + 1;
            }
            else if (arr[i].tech == ".Net") {
                abuKhadeeja.dotNet_Devs = abuKhadeeja.dotNet_Devs + 1;
            }
            else if (arr[i].tech == "Java") {
                abuKhadeeja.java_Devs = abuKhadeeja.java_Devs + 1;
            }
            else if (arr[i].tech == "Python") {
                abuKhadeeja.python_Devs = abuKhadeeja.python_Devs + 1;
            }
            if (((arr[i].firstName == '' || arr[i].firstName == null)&&(arr[i].lastName == '' || arr[i].lastName == null)) ||  arr[i].yearsOfExperience <= 1 ) {
                abuKhadeeja.rejectedApplicants = abuKhadeeja.rejectedApplicants + 1;
            }
    }
    return abuKhadeeja;
};
